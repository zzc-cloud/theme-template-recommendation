...
本模块封装主题下模板查询和覆盖率计算：根据用户已匹配的指标别名，评估每个
模板能覆盖多少分析需求。Skill 使用该结果判断是否有达标模板可推荐，或在没有
达标模板时给出降级说明和候选模板明细。
"""
...
def get_theme_templates_with_coverage(
    ...
    覆盖率 = 模板包含的用户匹配指标别名数 / 用户匹配指标别名总数。
    覆盖率达到 `TEMPLATE_COVERAGE_THRESHOLD` 的模板进入正式推荐；否则返回
    `fallback_reason` 和扫描过的模板详情，供 Skill 解释为什么只能降级展示。
    ...
    try:
        ...
        with get_neo4j_driver().session() as session:
            ...
            logger.info(f"[get_theme_templates_with_coverage]    user_indicator_set size={user_indicator_count}")
