...
本模块封装 Skill 需要的 Neo4j 本体图谱访问能力：按匹配指标聚合候选主题、
重建主题完整路径、获取主题下筛选/分析指标，以及沿“自主分析”根节点进行
板块和主题导航。API 层不直接调用这些函数，它们通过 tool_registry 暴露给
DeepAgents Skill 使用。
"""
...
# ─────────────────────────────────────────────
# Neo4j 驱动（单例）
# 工具函数调用频繁，驱动在进程内复用；应用关闭时由 main.py 统一释放。
# ─────────────────────────────────────────────
_neo4j_driver = None
...
def aggregate_themes_from_indicators(
    ...
    """
    从指标列表中聚合候选主题（按命中频次排序）。

    这是主题推荐的第一步：Skill 已通过向量搜索得到候选指标，本函数把这些指标
    映射回 Neo4j 中的 THEME 节点，并同时重建从“自主分析”到主题的展示路径。
    ...
    try:
        ...
        # 批量查询：一次性获取所有指标关联的 THEME 信息及展示路径，避免逐指标查库。
        business_labels = [
        ...
        with get_neo4j_driver().session() as session:
            # 查询从“自主分析”根节点到指标的完整路径，再截取其中的 THEME 节点。
            # theme_path 是最终推荐结果中帮助用户定位魔数师主题的关键展示字段。
            cypher = """
...
def get_theme_full_path(theme_id: str) -> dict:
    ...
    主题 ID 本身不适合直接展示给用户；完整路径能帮助用户在魔数师平台中按
    板块/分类/子路径快速定位主题。
...
# ─────────────────────────────────────────────
# 获取主题下的筛选指标
# 筛选指标用于把用户问题中的机构、时间等条件落到魔数师可勾选字段。
# ─────────────────────────────────────────────
