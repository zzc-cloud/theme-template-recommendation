...
1. 统计聚合路径：调用 `aggregate_themes_from_indicators(matched_indicators)`，按指标归属聚合候选主题。
2. 层级导航路径：调用 `get_sectors_from_root()`，选择相关板块后调用 `get_sector_themes(sector_id)`，基于主题名称和完整路径筛选候选主题。
3. 合并候选主题：按 `theme_id` 去重，保留统计聚合和层级导航两条路径的理由。
4. 指标补全：对每个候选主题调用 `get_theme_filter_indicators(theme_id)` 和 `get_theme_analysis_indicators(theme_id)`。
5. 主题裁决：判断主题业务领域、分析指标、筛选指标是否支撑用户需求；不要求主题预置聚合指标，无预聚合指标不能直接判定不支持。

推荐主题展示内容必须包含：主题名称、主题路径、支持理由、建议勾选的筛选指标和分析指标。
