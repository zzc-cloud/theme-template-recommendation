...
使用方法：
    python scripts/indicator_vectorizer.py --rebuild  # 重建向量索引
    python scripts/indicator_vectorizer.py --update   # 更新新增指标
    python scripts/indicator_vectorizer.py --stats    # 查看统计信息
    python scripts/indicator_vectorizer.py --search "关键词"  # 搜索演示
"""
...
# 性能参数
# ─────────────────────────────────────────────
API_BATCH_SIZE = 512   # 每次请求携带的文本条数
...
def get_embedding_batch(
    ...
    if not SILICONFLOW_API_KEY:
        raise EnvironmentError(
            "未设置 SILICONFLOW_EMBEDDING_API_KEY，请在 .env 文件中配置:\n"
            "SILICONFLOW_EMBEDDING_API_KEY=sk-xxxxxxxx"
        )
...
def vectorize_and_store(rebuild: bool = False) -> None:
    ...
    # ── 1. 检查 API Key ──────────────────────────
    if not SILICONFLOW_API_KEY:
        print("❌ 错误: 未设置 SILICONFLOW_EMBEDDING_API_KEY 环境变量")
        print("   请在 .env 文件中配置: SILICONFLOW_EMBEDDING_API_KEY=sk-xxxxxxxx")
        sys.exit(1)
