...
def get_streaming_llm_client() -> ChatOpenAI:
    """获取支持流式的 LLM 客户端（每次创建新实例避免状态污染）"""
    return ChatOpenAI(
        model=config.LLM_MODEL,
        api_key=config.SILICONFLOW_API_KEY,
        base_url=config.SILICONFLOW_BASE_URL,
...
def extract_phrases(user_question: str) -> PhraseExtraction:
    """阶段 0.1：提取词组"""
    from . import prompts as llm_prompts

    system_prompt = "你是一个专业的银行数据分析助手，擅长提取用户问题中的关键业务词组。重要：直接输出 JSON，不要使用任何 markdown 代码块（如 ```json）包裹。"
