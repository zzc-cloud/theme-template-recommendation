    ...
    for idx, theme in enumerate(recommended_themes):
        ...
        if result.get("success"):
            ...
            search_entry = {
                ...
                # 存储达标模板数据（热度>0 + 覆盖率>=阈值），后续 LLM 评估结果会回填
                "raw_templates": templates_for_analysis,
            }
            ...
            for t in templates_for_analysis:
                template_entry = {
                    ...
                    "has_qualified_templates": has_qualified,
                    "fallback_reason": fallback_reason,
                    # 保留模板全量指标数据，供 LLM 评估使用
                    "all_template_indicators": t.get("all_template_indicators", []),
                }
...
def analyze_templates(state: AgentState) -> dict:
    ...
    # 构建 template_id → usability 映射，用于回填 template_search_detail 中的 raw_templates
    template_usability_map: dict[str, dict] = {}
    ...
    for i, t in enumerate(templates):
        ...
        if tid:
            template_usability_map[tid] = usability

    # 将 LLM 评估结果回填到 template_search_detail 中的 raw_templates
    template_search_detail = state.get("template_search_detail", [])
    for detail_entry in template_search_detail:
        for raw_t in detail_entry.get("raw_templates", []):
            raw_tid = raw_t.get("template_id", "")
            if raw_tid and raw_tid in template_usability_map:
                raw_t["usability"] = template_usability_map[raw_tid]
...
def format_output(state: AgentState) -> dict:
    ...
    # 使用 template_search_detail 中的 raw_templates（热度>0 + 覆盖率>=阈值的达标模板），
    # 而非 recommended_templates（已被 LLM 过滤），确保输出包含所有达标模板及其评估理由
    template_search_detail = []
    for d in state.get("template_search_detail", []):
        theme_id = d.get("theme_id", "")
        # raw_templates 已在 analyze_templates 阶段被回填了 LLM 评估结果
        raw_templates = d.get("raw_templates", [])
        entry = {
            "theme_id": theme_id,
            ...
            # 该主题下所有达标模板（热度>0 + 覆盖率>=阈值，含 LLM 评估结果和支持理由）
            "templates": [
                {
                    "template_id": t.get("template_id", ""),
                    "template_alias": t.get("template_alias", ""),
                    ...
                    "matched_count": len(t.get("covered_indicator_aliases", [])),
                    "total_user_indicators": len(t.get("covered_indicator_aliases", [])) + len(t.get("missing_indicator_aliases", [])),
                    # LLM 评估结果（由 analyze_templates 回填）
